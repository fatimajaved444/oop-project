...
//		const int cnum = 8;
//
//		string c[cnum] = { "oop","discrete","coal","data","cps","linear","dld","de" };
//		rcourses = new string[max];
//
//		ifstream file("jav.txt");
//		if (!file.is_open())
...
//			istringstream c(line);
//			int roll;
//			string contact, age, name1, name2, crs;
//			if (c >> roll >> contact >> age >> name1 >> name2 >> crs)
//			{
//				if (roll == r)
//				{
//					if (roll == r)
...
//						studentfound = true;
//
//						// Find the position of the course to be removed
//						size_t pos = crs.find(courseToWithdraw);
//						if (pos != string::npos)
//						{
//							// Check if there is a space after the course, if not, add one
//							if (pos + courseToWithdraw.length() < crs.length() && !isspace(crs[pos + courseToWithdraw.length()]))
//							{
//								crs.insert(pos + courseToWithdraw.length(), 1, ' ');
//							}
//
//							// Remove the specified course
//							crs.erase(pos, courseToWithdraw.length());
//						}
//
//						// If courses become empty, set it to "No Courses"
//						if (crs.empty())
//						{
//							crs = "No Courses";
//						}
//					}
//				}
//				outfile << roll << "		" << contact << "		" << age << "		" << name1 << name2 << "			" << crs << endl;
//			}
//			outfile << line << '\n';
//		}
//		file.close();
